...
In it I calculate percent agreement within-observer across modalities
...
```{r}
## Sutherland photo data

sutherland <- ls_photos%>%
  filter(cranium != "h") %>%
...
for(i in 1:ncol(anderson_orbit_binary)){
  ...
  if(i == j) 0
   aa_orbit_irr[i,j] <- (agree(anderson_orbit_binary[,c(i,j)])$value )
  }
}
```
...
```{r}
aa_vault_irr <- matrix(NA, ncol = 4, nrow = 4)
# assign names to the matrix rows and columns
...
gc_vault_irr <- matrix(NA, ncol = 3, nrow = 3)
# assign names to the matrix rows and columns
...
ls_vault_binary <- sutherland %>%
  select(vault_photo_binary,
        combo_porosity,
        vault_scan_binary)

## Sutherland intra-observer agreement, vault scores
ls_vault_irr <- matrix(NA, ncol = 3, nrow = 3)
# assign names to the matrix rows and columns
rownames(ls_vault_irr) <- c("photo",
                            "2D CT",
                            "3D CT")
colnames(ls_vault_irr) <- rownames(ls_vault_irr)

# for loop for agree() for Sutherland's vault observations across modalities
for(i in 1:ncol(ls_vault_binary)){
  for(j in 1:ncol(ls_vault_binary)){
  if(i == j) 0
   ls_vault_irr[i,j] <- (agree(ls_vault_binary[,c(i,j)])$value / 100)
    }
}


# and correlation matrix plot for Sutherland's vault scores:
png('corrplot_vaults_ls.png')
vault_corrplot_ls <- corrplot(corr = ls_vault_irr, 
                            method = "color",
                            type = "lower",
                     col = colorRampPalette(c("green", "royal blue","red"))(100),
                     diag = TRUE,
